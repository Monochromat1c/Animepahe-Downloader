...
class AnimepaheGui(QWidget):
    ...
    def refresh_anime_list(self):
        ...
        proc = subprocess.run(
            ["C:\\Program Files\\Git\\bin\\bash.exe", "animepahe-dl.sh", "--list"],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        if proc.returncode == 0:
            self.status_label.setText("Anime list refreshed successfully.")
            self.log("[INFO] Anime list updated!")
        else:
            self.status_label.setText("Error refreshing anime list.")
